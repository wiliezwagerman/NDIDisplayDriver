...
void SwapChainProcessor::RunCore()
{
    ...
    for (;;)
    {
        ...
        else if (SUCCEEDED(hr))
        {
            ...
            // 
            //
            sendVideo(AcquiredBuffer, m_Device);
            // ==============================
...
static void sendVideo(ComPtr<IDXGIResource> buffer, shared_ptr<Direct3DDevice> Device) {
    IDXGIResource* Buffer = buffer.Get();

    Direct3DDevice device = Device->Device

    HRESULT hr = device->QueryInterface(__uuidof(IDXGIResource), (void**)&buffer);
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"NdiMonitor Device";
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"Microsoft";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"NdiMonitor Model";
